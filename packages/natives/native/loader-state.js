...
function getNativesDir() {
	...
	if (xdgDataHome && fs.existsSync(path.join(xdgDataHome, "ohms"))) {
		return path.join(xdgDataHome, "ohms", "natives");
	}
	return path.join(os.homedir(), ".ohms", "natives");
}
 ...
 * into the per-version cache directory (`~/.ohms/natives/<version>/`) before loading.
 *
...
export function initLoaderContext(overrides = {}) {
	...
	const userDataDir =
		platform === "win32"
			? path.join(process.env.LOCALAPPDATA || path.join(os.homedir(), "AppData", "Local"), "ohms")
