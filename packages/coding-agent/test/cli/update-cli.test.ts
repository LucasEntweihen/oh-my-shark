	...
	it("checks release metadata with a timeout signal", async () => {
		...
		const fetchStub = Object.assign(
			async (_input: FetchInput, init?: FetchInit) => {
				...
				return Response.json({ tag_name: "ohms-v999.0.0", draft: false, prerelease: false });
			},
...
describe("getLatestRelease fork releases", () => {
	...
	it("resolves the fork's latest stable release without touching a registry", async () => {
		const urls = stubReleases({ tag_name: "ohms-v0.0.3", draft: false, prerelease: false });

		const release = await getLatestRelease();

		expect(release).toEqual({ tag: "ohms-v0.0.3", version: "0.0.3", dist: "binary" });
		expect(urls).toEqual(["https://api.github.com/repos/LucasEntweihen/oh-my-shark/releases/latest"]);
	});
...
describe("getLatestRelease proxy errors", () => {
	...
	it("translates Bun's UnsupportedProxyProtocol fetch failure into an actionable CLI message", async () => {
		const fetchStub = Object.assign(
			async () => {
				throw new Error(
					'UnsupportedProxyProtocol fetching "https://api.github.com/repos/LucasEntweihen/oh-my-shark/releases/latest". ' +
