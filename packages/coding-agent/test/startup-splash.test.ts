...
import { SHARK_LOGO, gradientLogo } from "@oh-my-pi/pi-coding-agent/modes/components/welcome";
...
describe("startup splash", () => {
	...
	it("renders responsive shark frames without clipping and always reserves the credit", () => {
		const cases = [
		...
		for (const { width, height, expectArt } of cases) {
			...
			expect(plain.join("\n")).toContain("made by LucasEntweihen");
	...
	it("uses only the downstream blue ramps in truecolor and ANSI-256 modes", () => {
		...
		const truecolor = gradientLogo(SHARK_LOGO).join("");
		...
		const ansi256 = gradientLogo(SHARK_LOGO).join("");
