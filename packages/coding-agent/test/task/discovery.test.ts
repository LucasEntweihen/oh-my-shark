...
async function writeOmpPluginAgent(home: string): Promise<void> {
	const userPluginsRoot = path.join(home, ".ohms", "plugins");
...
describe("discoverAgents", () => {
	...
	test("loads OMP agents but skips Claude Code custom agents", async () => {
		await fs.mkdir(path.join(projectDir, ".ohms", "agents"), { recursive: true });
		await fs.writeFile(path.join(projectDir, ".ohms", "agents", "omp-test-agent.md"), OMP_AGENT_MD);
		...
		expect(projectAgentsDir).toBe(path.join(projectDir, ".ohms", "agents"));
	});

	test("loads agents from OMP npm plugins under <home>/.ohms/plugins/node_modules", async () => {
