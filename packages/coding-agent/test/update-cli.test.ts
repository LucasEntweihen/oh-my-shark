...
describe("parseReportedVersion", () => {
	it("preserves the prerelease suffix so a canary launcher verifies as up to date", () => {
		...
		expect(parseReportedVersion("ohms/0.0.1-canary.1")).toBe("0.0.1-canary.1");
		expect(parseReportedVersion("ohms/0.0.5")).toBe("0.0.5");
...
describe("update-cli binary-only target", () => {
	it("always updates in place, regardless of install layout or symlinks", () => {
		...
		expect(
			resolveUpdateMethodForTest("/Users/test/.bun/install/global/bin/ohms", "/Users/test/.bun/install/global/bin"),
		).toBe("binary");
		expect(resolveUpdateMethodForTest("/nix/store/0123456789-ohms-0.0.1/bin/ohms", undefined)).toBe("binary");
		expect(resolveUpdateMethodForTest("C:/Users/test/AppData/Local/ohms/ohms.exe", undefined)).toBe("binary");
	});
});

describe("parseProductTag", () => {
	it("accepts ohms-v<semver> tags", () => {
		expect(parseProductTag("ohms-v0.0.3")).toBe("0.0.3");
	});
	...
	it("rejects upstream and malformed tags", () => {
		...
		expect(parseProductTag("ohms-0.0.1")).toBeUndefined();
		expect(parseProductTag("ohms-v0.0")).toBeUndefined();
...
describe("getLatestRelease fork resolution", () => {
	...
	it("resolves the fork's latest stable release as a binary dist", async () => {
		...
		const release = await getLatestRelease({
			fetchImpl: releaseFetch({ tag_name: "ohms-v0.0.3", draft: false, prerelease: false }, seen),
			githubToken: "",
		});
	...
	it("reports that no canary channel exists without any network request", async () => {
		const seen: string[] = [];
		await expect(
			getLatestRelease({ channel: "canary", fetchImpl: releaseFetch({ tag_name: "ohms-v0.0.3" }, seen) }),
...
describe("update-cli never contacts upstream", () => {
	const tag = "ohms-v0.0.3";
	const binaries = [
		"ohms-darwin-x64",
		"ohms-darwin-arm64",
		"ohms-linux-x64",
		"ohms-linux-arm64",
		"ohms-windows-x64.exe",
		"ohms-windows-arm64.exe",
	];
	...
	function forkRelease(): Record<string, unknown> {
		return {
			...
			assets: binaries.map(name => ({
				...
				browser_download_url: `https://github.com/LucasEntweihen/oh-my-shark/releases/download/${tag}/${name}`,
			})),
		};
	}
	...
	it("resolves every release asset from the fork, with no upstream-shaped URL", () => {
		...
		for (const binaryName of binaries) {
			...
			expect(asset.url).toBe(`https://github.com/LucasEntweihen/oh-my-shark/releases/download/${tag}/${binaryName}`);
		}
	});
	...
	it("only ever requests fork URLs during a full check-and-download cycle", async () => {
		...
		const fetchImpl = async (input: string | URL | Request): Promise<Response> => {
			...
			if (
				requestUrl === "https://api.github.com/repos/LucasEntweihen/oh-my-shark/releases/latest" ||
				requestUrl === `https://api.github.com/repos/LucasEntweihen/oh-my-shark/releases/tags/${tag}`
			) {
		...
		const targetPath = path.join(dir, "ohms-linux-x64");
		await updateViaBinaryAt(targetPath, "0.0.3", {
			binaryName: "ohms-linux-x64",
			fetchImpl,
		...
		for (const requestUrl of seen) {
			expect(requestUrl).toContain("LucasEntweihen/oh-my-shark");
...
describe("update-cli release binary integrity", () => {
	const tag = "ohms-v0.0.3";
	const binaryName = "ohms-linux-x64";
	const url = `https://github.com/LucasEntweihen/oh-my-shark/releases/download/${tag}/${binaryName}`;
	...
	it("rejects a draft, a stable-channel prerelease, and metadata without one exact asset", () => {
		...
		expect(() =>
			resolveReleaseBinaryAsset(
				releaseAsset({ browser_download_url: "https://example.com/ohms-linux-x64" }),
				tag,
				binaryName,
			),
...
describe("update-cli fork release contract", () => {
	it("pins asset URLs to the fork's ohms-v tag", () => {
		const binaryName = "ohms-darwin-arm64";
		const tag = "ohms-v0.0.3";
		const url = `https://github.com/LucasEntweihen/oh-my-shark/releases/download/${tag}/${binaryName}`;
		const digest = `sha256:${createHash("sha256").update("shark").digest("hex")}`;
		expect(
			resolveReleaseBinaryAsset(
				{
					tag_name: tag,
...
describe("update-cli script-shim takeover", () => {
	// Tests that execute a text-file `ohms.exe` stub are skipped on Windows:
	...
	const version = "0.0.3";
	const binaryName = "ohms-windows-x64.exe";
	const url = `https://github.com/LucasEntweihen/oh-my-shark/releases/download/ohms-v${version}/${binaryName}`;
	...
	const shims: Record<string, string> = {
		ohms: "#!/bin/sh\nnode ohms.js\n",
		"ohms.cmd": "@node ohms.js %*\n",
		"ohms.ps1": "node ohms.js @args\n",
	};
	...
	it.skipIf(process.platform === "win32")("installs ohms.exe beside the shims and retires them", async () => {
		const dir = await makeTempDir();
		...
		await updateViaShimTakeover(path.join(dir, "ohms.cmd"), version, {
			binaryName,
			fetchImpl: makeFetch(exe),
			githubToken: "test-token",
		});

		expect(await Bun.file(path.join(dir, "ohms.exe")).text()).toBe(exe);
		for (const name in shims) {
	...
	it.skipIf(process.platform === "win32")(
		...
		async () => {
			...
			await expect(
				updateViaShimTakeover(path.join(dir, "ohms.cmd"), version, {
					binaryName,
					fetchImpl: makeFetch(exe, true),
					githubToken: "test-token",
				}),
			).rejects.toThrow("is a prerelease");
			expect(await Bun.file(path.join(dir, "ohms.exe")).exists()).toBe(false);
	...
	it("replaces the launcher in place with the verified binary, ignoring any manager metadata", async () => {
		...
		// `ohms.bunx` beside the launcher is inert and left untouched while
		// the launcher itself is replaced with the verified release binary.
		const dir = await makeTempDir();
		const targetPath = path.join(dir, "ohms.exe");
		const marker = path.join(dir, "ohms.bunx");
	...
	it.skipIf(process.platform === "win32")(
		...
		async () => {
			...
			).rejects.toThrow(/still reports 0\.0\.0 \(expected 0\.0\.3\); restored previous ohms launcher/);
	...
	function renameLockingPs1(): Mock<typeof nodeFs.promises.rename> {
		...
		return spyOn(nodeFs.promises, "rename").mockImplementation(async (from, to) => {
			if (path.basename(String(from)) === "ohms.ps1") {
	...
	it.skipIf(process.platform === "win32")(
		...
		async () => {
			...
			expect(await Bun.file(path.join(dir, "ohms.exe")).text()).toBe(exe);
			expect(await Bun.file(path.join(dir, "ohms")).exists()).toBe(false);
			expect(await Bun.file(path.join(dir, "ohms.cmd")).exists()).toBe(false);
			// PowerShell resolves .ps1 before .exe: the locked shim must now exec
			// the new binary instead of keeping its old body.
			expect(await Bun.file(path.join(dir, "ohms.ps1")).text()).toContain('& "$PSScriptRoot\\ohms.exe" @args');
		},
	);
