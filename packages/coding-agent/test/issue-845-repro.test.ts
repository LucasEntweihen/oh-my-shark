...
//
// OhMyShark updates are binary-only: there are no manager channels left to
...
describe("issue-845: binary-only updates ignore symlinked bin dirs", () => {
	it("classifies ohms reached through a symlinked bin dir as binary", () => {
		expect(resolveUpdateMethodForTest("/tmp/link-bin/ohms", "/tmp/real/bin")).toBe("binary");
	});

	it("classifies ohms at the real bin dir as binary when bunBinDir is symlinked", () => {
		expect(resolveUpdateMethodForTest("/tmp/real/bin/ohms", "/tmp/link-bin")).toBe("binary");
	});
});
