...
import { SHARK_LOGO, SHARK_WORDMARK, gradientEscape, gradientLogo, type ShineConfig } from "../../components/welcome";
import { theme } from "../../theme/theme";

export const SETUP_SPLASH_MS = 2600;
export const SETUP_TICK_MS = 33;

const LOGO_WIDTH = Math.max(...SHARK_LOGO.map(line => visibleWidth(line)));
const LOGO_HEIGHT = SHARK_LOGO.length;
const RESET = "\x1b[0m";
...
/**
 * Animated setup splash: the blue shark mark rises out of a rippling,
...
export function renderSetupSplash(width: number, height: number, elapsedMs: number): string[] {
	...
	// 3. hero — the 1x shark mark with the live gradient + shine sweep
	SHARK_LOGO.forEach((line, row) => {
		let col = 0;
...
function renderCompactSplash(width: number, height: number, phase: number, shine: ShineConfig): string[] {
	...
	const content = showArt
		? [...gradientLogo(SHARK_LOGO, phase, shine), "", theme.bold(SHARK_WORDMARK)]
		: [theme.bold(SHARK_WORDMARK)];
