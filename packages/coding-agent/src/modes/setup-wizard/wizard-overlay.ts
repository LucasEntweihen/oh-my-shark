...
import { SHARK_LOGO, SHARK_WORDMARK, gradientLogo } from "../components/welcome";
...
export class SetupWizardComponent implements Component, OverlayFocusOwner {
	...
	#renderScene(width: number, height: number): string[] {
		...
		const logo = height >= SHARK_LOGO.length + 18 ? gradientLogo(SHARK_LOGO, 0) : [theme.bold(SHARK_WORDMARK)];
		const header = [
			"",
