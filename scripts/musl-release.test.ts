...
describe("musl release artifacts", () => {
	test("builds the requested x64 and arm64 musl asset names with Bun's musl targets", async () => {
		...
		expect(result.stdout).toContain(
			"Bun.build target=bun-linux-x64-musl-baseline outfile=packages/coding-agent/binaries/ohms-linux-musl-x64",
		);
		expect(result.stdout).toContain(
			"Bun.build target=bun-linux-arm64-musl outfile=packages/coding-agent/binaries/ohms-linux-musl-arm64",
		);
	});

	test("selects the musl asset when the Linux host reports musl", async () => {
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "ohms-musl-install-"));
		tempDirs.push(dir);
...
case "$*" in
  *SHA256SUMS.txt*) while [ "$#" -gt 0 ]; do
       [ "$1" = "-o" ] && { printf '%b' '${binaryDigest}  ohms-linux-musl-x64\\n' > "$2"; exit 0; }
       shift
     done ;;
  *) while [ "$#" -gt 0 ]; do
       [ "$1" = "-o" ] && { printf '%b' '#!/bin/sh\\necho "ohms v0.0.3"\\n' > "$2"; exit 0; }
...
`,
		...
		expect(result.exitCode, result.stderr).toBe(0);
		expect(result.stdout).toContain("Using version: ohms-v0.0.3");
		expect(result.stdout).toContain("Downloading ohms-linux-musl-x64...");
		expect(result.stdout).toContain("Checksum OK");
		expect(await Bun.file(path.join(installDir, "ohms")).text()).toBe(binaryContent);
	});
});
