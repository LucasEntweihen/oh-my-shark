...
omp plugin <npm/link action> ...
  ...
  -> mutate user plugins data root {package.json,node_modules,ohms-plugins.lock.json}
...
omp plugin install name@marketplace / omp install name@marketplace
  ...
  -> symlink the cached package into the scope's node_modules and update ohms-plugins.lock.json
...
- `ohms-plugins.lock.json` — runtime state for npm/link/marketplace plugins:
  - enabled/disabled per plugin
  - selected feature set per plugin
  - persisted plugin settings

When a project anchor (`.omp/` or `.git/`) exists at or above cwd, project runtime plugins live in `<anchor>/.omp/plugins/{node_modules,ohms-plugins.lock.json}`. Marketplace project installs populate this root; enabled project packages shadow user packages with the same package name.
...
- `<scope>/plugins/ohms-plugins.lock.json` — enablement and feature state shared with the runtime plugin loader
