# OhMyShark Coding Agent Installer for Windows
# Verify first: gh attestation verify install.ps1 --repo LucasEntweihen/oh-my-shark --signer-workflow LucasEntweihen/oh-my-shark/.github/workflows/release-ohms.yml --source-ref refs/tags/ohms-v0.0.3 --deny-self-hosted-runners
# Then run: & ([scriptblock]::Create((Get-Content .\install.ps1 -Raw)))
# Or quick: irm https://ohmyshark.vercel.app/install.ps1 | iex
#
# Or with options:
#   & ([scriptblock]::Create((Get-Content .\install.ps1 -Raw))) -Binary
#   & ([scriptblock]::Create((Get-Content .\install.ps1 -Raw))) -Source
#   & ([scriptblock]::Create((Get-Content .\install.ps1 -Raw))) -Source -Ref ohms-v0.0.3
#   & ([scriptblock]::Create((Get-Content .\install.ps1 -Raw))) -Binary -Ref ohms-v0.0.3
...
$BinaryName = "ohms-windows-$NativeArchitecture.exe"
...
function Configure-BashShell {
    try {
        $settingsDir = Join-Path $env:USERPROFILE ".ohms\agent"
        ...
        if ($bashPath) {
            ...
            Write-Host "No bash shell found - ohmyshark will use its built-in shell." -ForegroundColor Cyan
...
function Install-ViaBun {
    ...
    $tmpRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("ohms-install-" + [System.Guid]::NewGuid().ToString("N"))
    ...
    try {
        ...
        if (Test-Path $SumsPath) {
            ...
            if ($assetMatch) {
                ...
                $assetActual = Get-OhmsFileHash -Path $tmp
        ...
        Write-Host "[WARN] Could not seed $leaf; ohms will create defaults on first run." -ForegroundColor Yellow
    } finally {
...
function Install-Binary {
    ...
    try {
        ...
        $Actual = Get-OhmsFileHash -Path $TmpPath
        if ($Actual -ne $Expected.ToLowerInvariant()) {
            throw "Checksum mismatch for ${BinaryName}: expected $Expected, actual $Actual. Aborting without touching the installed ohms."
        }
        ...
        if (-not (Test-Path $configYml) -and -not (Test-Path (Join-Path $AgentDir "config.yaml"))) {
            Seed-Preset -Tag $Tag -Asset "ohmyshark.config.yml" -Destination $configYml -SumsPath $SumsPath
        } else {
            Write-Host "Keeping existing agent config"
        }
        Seed-Preset -Tag $Tag -Asset "ohmyshark.models.yml" -Destination (Join-Path $AgentDir "models.yml") -SumsPath $SumsPath
    } finally {
