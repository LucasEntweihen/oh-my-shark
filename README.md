...
One command installs the `ohms` CLI on Windows or macOS. Models arrive
...
[Can Bölük](https://github.com/can1357) / Stencil Labs → **OhMyShark** by
LucasEntweihen
...
```sh
# macOS / Linux
curl -fsSL https://ohmyshark.vercel.app/install | sh
```
...
- Everything lives in plain YAML under `~/.ohms/agent` (macOS/Linux) or
  `%USERPROFILE%\.ohms\agent` (Windows) — isolated, never mixed with other tools.
...
> credit upstream and minimize merge conflicts. Where it says `omp`, read `ohms`; where
> it links `omp.sh`, the OhMyShark equivalents are [ohmyshark.vercel.app](https://ohmyshark.vercel.app)
> and this repository.
